// myGemmDouble.c
//
// This program implements a parallel matrix multiplication using MPI.
...
//#define INTERACTIVE
//#define DEBUG
void blockMultiply(const int,const int,const int,double*,double*,double*,int,int,double*,MPI_Status);
void raj(double*,double*,double*,int,int,int,int,int,int,const int);
void initMatrices(double*,double*,double*,int);
...
//        printf("Partition: %d\n",partition);
    ...
    for(x=initialSize;x<=finalSize;x*=2)
    {
        if(x>=4096)
            iterations=3;
        a=malloc(x * x * sizeof(double));
        b=malloc(x * x * sizeof(double));
        c=malloc(x * x * sizeof(double));
	if(rank==0)
...
void blockMultiply(const int NSIZE,const int NBSIZE,const int ITERATIONS,double *a,double *b,double *c,int size,int rank,double *arrayOfTime,MPI_Status status)
{
    int u,v,w,x,y;
    for(x=0;x<ITERATIONS;x++)
    {
        ...
        if(rank==0)
        {
            ...
            for(y=1;y<size;y++)
            {
                MPI_Send(&a[y*NSIZE*NSIZE/size],NSIZE*NSIZE/size,MPI_double,y,0,MPI_COMM_WORLD);
                MPI_Send(b,NSIZE*NSIZE,MPI_double,y,1,MPI_COMM_WORLD);
            }
            #ifdef DEBUG
            ...
            for(y=1;y<size;y++)
            {
                ...
                #endif
                MPI_Recv(&c[y*NSIZE*NSIZE/size],NSIZE*NSIZE/size,MPI_double,y,0,MPI_COMM_WORLD,&status);
            }
...
inline void raj(double *a, double *b, double *c, int rowStartI,int rowStartJ, int rowStartK,int endI, int endJ,int endK,const int NSIZE)
{
    int i,j,k;
...
void initMatrices(double *h,double *i, double *j,int NSIZE)
{
    int x;
...
void printMatrix(double *a,const int NSIZE)
{
    int i;
